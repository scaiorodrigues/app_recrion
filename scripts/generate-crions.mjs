  ...
  crions.push({
    ...
    attacks: attacks.map(([name, element, power, accuracy, unlockXP, description, effect], i) => ({
      ...
      art: {
        ...
        full: `trading card creature artwork, square 1:1 composition, subject centred with margin on all sides. Subject: legendary fantasy creature, ${rest.baseEmoji === '🦄' ? 'celestial chimera' : 'twilight wolf'}, ${SLOT_POSE[i]}. Surrounding it: ${SLOT_EFFECT_INTENSITY[i]} of radiant prismatic energy. Behind it: epic legendary fantasy landscape with soft depth of field. Around the edges: the same radiant light bleeding outward into a soft vignette. Children illustration style, vibrant colors, no text, no lettering, no border frame`,
      },
    })),
  });
}
